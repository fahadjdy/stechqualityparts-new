...
<x-app-layout>
    ...
    <section class="pt-32 pb-16 bg-slate-50">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <p class="text-slate-500 mt-3 max-w-xl">Have questions about our parts or need a bulk order quote? Our
                ...
                <div class="space-y-5">
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                        <div class="metallic-card p-6 space-y-4 bg-white !rounded-xl">
                            ...
                            <a href="tel:{{ $profile->contact ?? '+918128912711' }}"
                                class="block text-blue-600 font-bold text-sm hover:translate-x-1 transition-transform">{{ $profile->contact ?? '+91 8128912711' }}</a>
                        </div>
                        <div class="metallic-card p-6 space-y-4 bg-white !rounded-xl">
                            <div class="w-11 h-11 rounded-xl bg-blue-50 flex items-center justify-center">
                            ...
                            <a href="mailto:{{ $profile->email ?? 'info@stechqualityparts.com' }}"
                                class="block text-blue-600 font-bold text-sm hover:translate-x-1 transition-transform">{{
                                $profile->email ?? 'info@stechqualityparts.com' }}</a>
                        </div>
                    </div>
                        ...
                        <div class="flex items-center gap-4">
                            ...
                            <div>
                                ...
                                <p class="text-slate-500 text-sm">
                                    {{ $profile->location ?? $profile->city ?? 'Gujarat, India' }}
                                </p>
                            </div>
                        </div>
                ...
                <div class="reveal metallic-card p-8 lg:p-10 !rounded-2xl bg-white shadow-2xl shadow-blue-500/5">
                    <form action="{{ route('inquiries.store') }}" method="POST" class="space-y-6">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div class="space-y-2">
                                <label
                                    class="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Email</label>
                        ...
                        <div class="space-y-2">
                            <label
                                class="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Subject</label>
                            <select name="subject"
                                class="form-input cursor-pointer appearance-none bg-no-repeat bg-[right_1.25rem_center] bg-[length:1em_1em]"
                        ...
                        <div class="space-y-2">
                            <label
                                class="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Message</label>
                            <textarea name="message" rows="4" placeholder="Tell us what you're looking for..." required
                                class="form-input resize-none"></textarea>
                        </div>
                        <button type="submit"
                            class="btn-primary w-full justify-center py-4 uppercase tracking-widest text-sm shadow-xl shadow-blue-500/20">
